...
class basic_conv(nn.Module):
    def __init__(self,
        ...
        self.out_channels = out_planes # 定义输出的通道数
        # 定义二维卷积
...
class CTPN_Model(nn.Module):
    def __init__(self):
        super().__init__()
        base_model = models.vgg16(pretrained=False) # 使用现有的网络模型vgg16进行训练，权重为pretrained
        layers = list(base_model.features)[:-1] # 得到vgg16的模型特征，然后去掉最后一层
        self.base_layers = nn.Sequential(*layers)  # block5_conv3 output
        self.rpn = basic_conv(512, 512, 3, 1, 1, bn=False) # 通过basic_conv的到数据
        self.brnn = nn.GRU(512,128, bidirectional=True, batch_first=True) # 循环神经网络
        self.lstm_fc = basic_conv(256, 512, 1, 1, relu=True, bn=False)# 通过basic_conv的到数据
        self.rpn_class = basic_conv(512, 10 * 2, 1, 1, relu=False, bn=False)# 通过basic_conv的到数据
        self.rpn_regress = basic_conv(512, 10 * 2, 1, 1, relu=False, bn=False)# 通过basic_conv的到数据
