...
def resize(image, width=None, height=None, inter=cv2.INTER_AREA): # image是三维数组形式
    # initialize the dimensions of the image to be resized and
    # grab the image size
    dim = None
    (h, w) = image.shape[:2] # 取出数组的前两位高和宽
    ...
    # 将图片调整为指定dim大小 差值方法使用像素区域关系进行重采样（有利于缩小图像）。
...
def gen_anchor(featuresize, scale):
    ...
    x2 = xt + widths * 0.5
    y2 = yt + heights * 0.5
    base_anchor = np.hstack((x1, y1, x2, y2)) # 在水平方向上平铺
...
def bbox_transfor_inv(anchor, regr):
    ...
    x2 = xt + 16 * 0.5
    y2 = Cyx + hx * 0.5
    bbox = np.vstack((x1, y1, x2, y2)).transpose() # 在竖直方向上堆叠 并且转置
...
def nms(dets, thresh):
    ...
    order = scores.argsort()[::-1] # 对数组从小到大排序 逆序输出
